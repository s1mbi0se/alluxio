 ...
 */
@ThreadSafe public class B2UnderFileSystemFactory implements UnderFileSystemFactory {
  @Override public UnderFileSystem create(String path, UnderFileSystemConfiguration conf) {
  ...
  private boolean checkB2Credentials(UnderFileSystemConfiguration conf) {
    return conf.keySet().contains(PropertyKey.B2_ACCESS_KEY) && conf.keySet()
        .contains(PropertyKey.B2_SECRET_KEY);
  }

  @Override public boolean supportsPath(String path) {
    if (path == null) {
      return false;
    }
  ...
  @Override public boolean supportsPath(String path, UnderFileSystemConfiguration conf) {
